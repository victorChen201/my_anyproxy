...
function Recorder(option){
    ...
    self.removeRecord = function(query){db.removeFromIndexes(query);};
    self.getAllData = function(){return db.getAllData();};
    //
    self.appendRecord = function(info){
        if(info.req.headers.anyproxy_web_req){ //request from web interface
            return -1;
        }
        // if (info['req']['IncomingMessage']['headers']['content-type']=='application/x-www-form-urlencoded'){
        // {    var querystring = require('querystring');
        //         info.reqbody = querystring.unescape(info.reqbody);
        // }
...
function normalizeInfo(id,info){
    ...
    singleRecord.testType = info.testType || "";
