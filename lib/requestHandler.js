var http = require("http"),
    ...
    reportHandler = require("./reportHandler");
    ...
    */
    ...
    function exReq(callback) {
        async.series([
            ...
            if (req.anyproxy_map_local) {
                global.recorder.updateExtInfo(resourceInfoId, {
                    map: req.anyproxy_map_local
                });
            }
            callback();
        });
    ...
    function fetchReqData(callback) {
        ...
        req.on("end", function() {
            ...
            /*by Dwin 20210410 对请求body进行解码*/
            var querystring = require('querystring');
            resourceInfo.reqBody = querystring.unescape(reqData.toString());
            // resourceInfo.reqBody = reqData.toString();
            /*by Dwin 20210410*/
            global.recorder && global.recorder.updateRecord(resourceInfoId, resourceInfo);
            // console.log(resourceInfo);
            // add by Dwin 20210414
            reportHandler.fetchReportData(resourceInfoId,resourceInfo);
            callback();
        });
    }

    //route to dealing function
    function routeReq(options, callback) {
        if (userRule.shouldUseLocalResponse(req, reqData)) {
            ...
            dealWithRemoteResonse(options, callback);
        }
    }
    ...
    function dealWithRemoteResonse(options, callback) {
        //var options;
            ...
            res.on("end", function() {
                ...
                async.series([
                    ...
                    function(callback) {
                        ...
                        resourceInfo.path = options.path
                        resourceInfo.testType = options.description

                        global.recorder && global.recorder.updateRecord(options.resourceInfoId, resourceInfo);
                    ...
                    function(callback) {
                        userRule.fetchTrafficData && userRule.fetchTrafficData(options.resourceInfoId, resourceInfo);
                        callback(null, '');
